...
> 🛠️ 2026-07-21 팀 FigJam(박소호) 기준으로 실 구현 페이지 목록 반영 — "학습 상세"를 요약본 노트 / 학습노트 상세로 분리, 가입완료·전체 요약 노트 페이지 추가.
...
9. 전체 요약 노트 페이지 — 전체 게시글 피드 (카테고리 필터, 무한스크롤), 아카이브 열람은 이 페이지가 담당
...
| 구분         | 선택                                                   | 비고                                         |
| ------------ | ------------------------------------------------------ | -------------------------------------------- |
| Framework    | React (Vite)                                           | Next.js 필수 여부 **TBD** — 강사 확인 필요   |
...
| 배포         | GitHub Pages / Vercel                                  | 명세 기준                                    |
| 개발 도구    | Claude Code, Context7, Chrome DevTools, Figma 플러그인 | 개인 작업 방식 (팀 공식 아님)                |
...
> ⚠️ **TBD — 팀 확인 필요**: 요약본 노트 페이지(퀴즈) ↔ 학습노트의 관계 — 후보 A: 퀴즈는 요약본 노트에 바로 붙고 노트 작성은 완전히 독립적(현재 가정) / 후보 B: 노트 작성 완료해야 퀴즈 접근 가능(게이팅) / 후보 C: 둘 다 독립적, 서로 무관
>   → 다음 팀 논의에서 확정 후 이 문서와 다이어그램 업데이트할 것
...
- 댓글 기능
- 모의고사/티어 시스템 (별도 분리, 이번 스코프에서 제외됨)
- 관리자 페이지 (명세상 옵션)
- 통계/차트 (chart.js 등)

### 이번 스코프 제외 (참고용 — 개인 프로젝트 후보)

- 개발자용 통합 스니펫/프롬프트/노트 저장 도구 (DevStash류 아이디어) — 별도 사이드 프로젝트로 고려 중
